...
import { bip39, hdkey, getAddress, BN, Signature } from '@astra-js/crypto';
import {
  HDPath,
  // defineReadOnly,
  isHttp,
...
import { Messenger, HttpProvider, WSProvider, RPCMethod } from '@astra-js/network';
import { Transaction, TxStatus, TransasctionReceipt } from '@astra-js/transaction';
...
export class HDNode {
  ...
  constructor(
    ...
    shardID: number = 0,
    chainType: ChainType = ChainType.Astra,
    ...
    this.addresses = [];
    this.wallets = {};
    this.path = chainType === ChainType.Astra ? HDPath : `m/44'/60'/0'/0/`;
    this.index = index;
