...
Available units
```
const { Units } = require('@astra-js/utils');
...
```javascript
const { Units, Unit, numberToString, add0xToString, fromWei, toWei, numToStr} = require('@astra-js/utils');
const { BN } = require('@astra-js/crypto');
...
// toWei
const one = new BN('1');
const expected = toWei(one, astra.utils.Units.one);
...
export const defaultConfig = {
  Default: {
    Chain_ID: ChainID.AstraLocal,
    Chain_Type: ChainType.Astra,
...
/** @hidden */
export abstract class AstraCore {
  chainType: ChainType;
  chainId: ChainID;
