...
import { add0xToString, numberToHex, ChainType, Unit } from '@astra-js/utils';
...
class Transaction extends TransactionBase {
  ...
  getRLPUnsigned(): [string, any[]] {
    ...
    const fields =
      this.messenger.chainType === ChainType.Astra ? transactionFields : transactionFieldsETH;
  ...
  getRLPSigned(raw: any[], signature: Signature): string {
    // temp setting to be compatible with eth
    const rawLength = this.messenger.chainType === ChainType.Astra ? 11 : 9;
  ...
  recover(rawTransaction: string): Transaction {
    // temp setting to be compatible with eth
    const recovered =
      this.messenger.chainType === ChainType.Astra
   ...
   *   shardID: 1,
   *   toShardID: 0,
   *   gasPrice: new astra.utils.Unit('101').asGwei().toWei(),
   *   signature: {
