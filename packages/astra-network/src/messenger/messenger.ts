...
import { AstraCore, ChainType, isString, ChainID, defaultConfig } from '@astra-js/utils';
 ...
 * ```
 */
class Messenger extends AstraCore {
  ...
  async setShardingProviders() {
    if (this.chainPrefix !== ChainType.Astra) {
      return;
    }
    try {
   ...
   * @example
   * ```
   * astra.messenger.getShardProvider()
   * ```
   */
   ...
   * @example
   * ```
   * astra.messenger.getCurrentShardID()
   * ```
   */
